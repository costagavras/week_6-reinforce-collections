...
# puts hh_articles[:response]

# i = 0
# hh_articles[:response][:results].each do |k|
#     k[:views] = 0
#     # i +=1
# end
#     puts hh_articles[:response][:results]

def add_views(my_hash)
  my_hash[:response][:results].each do |k|
      k[:views] = 0
  end
  return my_hash
end

def read_article(my_hash, num_reads)
  ar_random = my_hash[:response][:results].sample
  ar_random[:views] += num_reads
  return ar_random
end

# puts read_article(hh_articles, 5)

def display_views(my_hash, num_articles_read)
  add_views(my_hash)
  num_articles_read.times do
    read_article(my_hash, 1)
  end
  my_hash[:response][:results].each do |k|
      puts "The article #{k[:webTitle]} has been viewed: #{k[:views]} times."
  end
end

#some articles are read several times
display_views(hh_articles, 9)
